...
//		response시에는 minetype과 encoding => contentType 지정을 해줘야 한글깨짐이 없음
...
//		MVC 중 model의 메소드를 호출하여 DB와 연동한 결과를 view에 response하게 됨.
		...
		if (result > 0) {
			System.out.println(response.getWriter().append("로그인 성공"));
		} else {
			System.out.println(response.getWriter().append("로그인 실패"));
		}
